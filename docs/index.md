...
1. **Traditional** — [`QMBackend.run()`](apidocs/stubs/qiskit_qm_provider.backend.QMBackend.rst) and V2 primitives ([`QMSamplerV2`](apidocs/stubs/qiskit_qm_provider.primitives.QMSamplerV2.rst), [`QMEstimatorV2`](apidocs/stubs/qiskit_qm_provider.primitives.QMEstimatorV2.rst)) compile Qiskit circuits to QUA and execute them on QOP, with optional real-time parameter streaming. For `backend.run` only, large circuit lists are chunked into multiple QUA programs via [`max_circuits`](apidocs/stubs/qiskit_qm_provider.backend.QMBackend.rst) (default 30); see [Backend guide](backend.md#multi-circuit-batches-and-max_circuits-backend-run-only).
