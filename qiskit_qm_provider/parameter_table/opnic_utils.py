...
"""OPNiC utilities: C++ type/packet helpers and parameter handling for QUA.
...
def make_cpp_packet_definition(
    ...
    for i, parameter in enumerate(parameters):
        packet_definition += parameter.name
...
def patch_opnic_wrapper(
    ...
    path_to_python_wrapper: Union[str, Path] = "/home/dpoulos/aps_demo",
    ...
    paths = {
        ...
        "h": path_to_python_wrapper / "wrapper/wrapper.h",
        "i": path_to_python_wrapper / "wrapper/wrapper.i",
        "parent": path_to_python_wrapper / "wrapper",
        "build_sh": path_to_python_wrapper / "wrapper/build.sh",
    }
    replacements = []

    for param_table in param_tables:
        direction: Literal["Incoming", "Outgoing"] = (
            "Incoming" if param_table.direction == Direction.INCOMING else "Outgoing"
        )
    ...
    if force_recompile_python_wrapper or modified_count > 0:
        if force_recompile_python_wrapper:
            logging.info(f"Recompiling python wrapper...")
        else:
            logging.info(f"{modified_count} changes detected. Recompiling...")
        ...
        print(f"No modified C++ files. Skipping compilation...")
