...
def test_attach_quarc_stream_specs_rejects_non_dgx():
    p = Parameter("x", 0.0)
    t = ParameterTable([p], name="t")
    with pytest.raises(ValueError, match="DGX_Q"):
        t._attach_quarc_stream_specs(incoming_id=1, outgoing_id=2)


def test_dgx_qua_edges_require_prepare():
    p = Parameter("x", 0.0, input_type=InputType.DGX_Q, direction=Direction.OUTGOING)
    t = ParameterTable([p], name="policy")
    with pytest.raises(RuntimeError, match="prepare_dgx_quarc_hybrid_packets"):
        t._stream_id_for_qua_incoming_edge()


def test_prepare_dgx_assigns_via_quarc_module():
    p1 = Parameter("a", 0.0, input_type=InputType.DGX_Q, direction=Direction.OUTGOING)
    t1 = ParameterTable([p1], name="t1")
    p2 = Parameter("b", 0.0, input_type=InputType.DGX_Q, direction=Direction.INCOMING)
    t2 = ParameterTable([p2], name="t2")
    ParameterPool.prepare_dgx_quarc_hybrid_packets()
...
def test_iter_dgx_parameter_tables_order():
    p1 = Parameter("a", 0.0, input_type=InputType.DGX_Q, direction=Direction.OUTGOING)
    t1 = ParameterTable([p1], name="t1")
    p2 = Parameter("b", 0.0, input_type=InputType.DGX_Q, direction=Direction.INCOMING)
    t2 = ParameterTable([p2], name="t2")
    assert [x._id for x in ParameterPool.iter_dgx_parameter_tables()] == sorted([t1._id, t2._id])


def test_default_quarc_struct_name_standalone_parameter():
    p = Parameter("solo", 1.0, input_type=InputType.DGX_Q, direction=Direction.OUTGOING)
    ParameterPool.prepare_dgx_quarc_hybrid_packets()
    assert default_quarc_struct_name(p) == f"Packet_solo_{p.stream_id}"


def test_prepare_standalone_both():
    p = Parameter("solo", 1.0, input_type=InputType.DGX_Q, direction=Direction.BOTH)
    ParameterPool.prepare_dgx_quarc_hybrid_packets()
...
def test_incoming_table_qua_outgoing_edge_uses_quarc_incoming_spec():
    p = Parameter("y", 0.0, input_type=InputType.DGX_Q, direction=Direction.INCOMING)
